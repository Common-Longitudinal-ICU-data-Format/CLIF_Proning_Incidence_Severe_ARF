...
base_plot <- ggplot(overall_data, aes(x = study_quarter, y = proned_12_hr_percent)) +
  ...
  # Add COVID-19 period separator with labels
  geom_vline(xintercept = 9.6667, linetype = 2) +
  annotate("text", x = 5.667, y = 1.025, label = 'Pre-Pandemic') +
  geom_vline(xintercept = 17.6667, linetype = 2) +
  annotate("text", x = 13.667, y = 1.025, label = 'Pandemic') +
  annotate("text", x = 21.667, y = 1.025, label = 'Post-Pandemic') +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))
# Display the plot
base_plot
...
base_plot <- ggplot(all_site, aes(x=study_quarter, y=proned_12_hr_percent)) +
  ...
  annotate("text", x = 5.0, y = 1.025, label = 'Pre-Pandemic') +
  geom_vline(xintercept = 17.6667, linetype = 2) +
  annotate("text", x = 13.667, y = 1.025, label = 'Pandemic') +
  annotate("text", x = 22.667, y = 1.025, label = 'Post-Pandemic') +
  theme_classic() +
  # Adjust legend font sizes in theme
  theme(
...
base_plot <- ggplot(subset(prone_per_quarter_covid, site =='all_sites'), 
  ...
  annotate("text", x = 5.0, y = 505, label = 'Pre-Pandemic', size=3.3) +
  geom_vline(xintercept = 17.5, linetype = 2) +
  annotate("text", x = 14.0, y = 505, label = 'Pandemic', size=3.3) +
  annotate("text", x = 21.667, y = 505, label = 'Post-Pandemic', size=3.3) +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1),
     legend.position = c(0.155, 0.75),
        legend.text = element_text(size = 8),  # Smaller legend text
...
tables <- c(
  ...
  "_prone_duration_categorical.csv"
)
...
```
```{r Prone Practice Categories}
table_prone_cat <- collate_tables("_prone_duration_categorical.csv") 

#Now Aggregate Categorical Variables
aggregate_outcomes <- table_prone_cat |>
  group_by(strata, variable, level) |>
  dplyr::reframe(
    n_total=sum(n-miss),
    n_miss=sum(miss, na.rm=T),
  ...
  #Take Max n_total within each strata; This is because if a category is missing from a site, it will not be included in the total N denominator
  group_by(strata) |>
  mutate(
    n_total=max(n_total),
...
#Order of Variables
order_vec <- table_prone_cat$variable
order <- data.frame(
  variable=unique(order_vec)) |>
  mutate(order_number=dplyr::row_number())
...
#Merge Back with Table 1 and Convert to Wide
table_prone_cat_paper <- aggregate_outcomes |>
  select(strata, variable, level, n_total, count, percent, order_number) |>
  mutate(
    percent=round((percent*100), digits = 2)
  ) |>
...
#Reorder and Add Statistical Testing
table_prone_cat_paper <- table_prone_cat_paper |>
  relocate(all_of(relocate_vector), .after = level) |>
  group_by(variable) |>
  arrange(variable) |>
  mutate(rn=dplyr::row_number()) |>
  ...
  arrange(order_number, variable, temp) |>
  select(-order_number, -rn, -temp) 
  
table_prone_cat_paper <- readable_table(table_prone_cat_paper, strata_values, has_continuous=F, has_categorical=T)

write_csv(table_prone_cat_paper, paste0(project_location, '/summary_output/prone_practices_categorical.csv'))
