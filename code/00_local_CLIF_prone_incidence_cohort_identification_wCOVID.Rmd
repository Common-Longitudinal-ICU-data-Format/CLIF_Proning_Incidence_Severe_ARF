...
prone_episodes <- prone_episodes_all |>
  ...
  mutate(keep=fifelse(
    ...
    position_category!=lag(position_category) & recorded_dttm==lag(recorded_dttm), 0, keep, 1)) |>
  ungroup() |>
  filter(keep==1) |>
...
sars_cov2 <- clif_microbiology_nonculture |>
  ...
  filter(organism_category %in% c('sars_cov2')) |>
  left_join(cohort_hospitalization_ids) |> #Merges in Encounter Block
  left_join(cohort_times) |>
  collect() |>
  filter(collect_dttm<=t_enrollment & collect_dttm>=t_enrollment-dweeks(4)) |>
  group_by(patient_id, encounter_block) |>
...
prone_analytic_data <- hospitalization_vars |>
  ...
  mutate(race_ethnicity=fcase(
    ...
    tolower(race_category) %in% c('black or african american', 'black or african-american') & 
