...
ame_summary <- function(model_list) {
  ...
  for (i in 1:length(model_list)) {
    ...
    if (grepl(":month_scaled", formula_string, ignore.case = TRUE)) {
...
prone_analytic_df <- prone_analytic_df |>
  mutate(
    age_scale=as.numeric(scale(age_at_admission)),
    bmi_scale=as.numeric(scale(bmi)),
    min_pf_ratio_scale=as.numeric(scale(min_pf_ratio)),
    sofa_score_scale=as.numeric(scale(sofa_score))
  )
```
...
#Agreed Upon Covariates (see DAG): Age, Gender, BMI, PF Ratio, Norepi Equivalents, LAPS2, OR Prior to Eligibility


#Many Sites Have Zero Patients Who were Proned and Were In OR Prior to Enrollment; For Those Sites that Variable is Not Included in the Model
#Build an Indicator On Whether to Include OR Before Enrollment in Models or Not
#Only Include if At Least 5 Patients Are Prone After OR
no_or <- prone_analytic_df |>
  mutate(or_num = sum(or_before_enrollment, na.rm = TRUE)) |>
  filter(prone_12hour_outcome==1) |>
  mutate(prone_after_or=sum(or_before_enrollment, na.rm=TRUE)) |>
  select(or_num, prone_after_or) |>
  slice(1)

no_or <- if (no_or$or_num<6 | no_or$prone_after_or<5) {
  1
} else {
  0
}
...
model_form_step <- prone_12hour_outcome ~ study_period + age_scale + female + bmi_scale + factor(nee_pressor_dose) + sofa_score_scale +  
  min_pf_ratio_scale + or_before_enrollment + factor(hospital_id)
#Now Modified for Each Site
if (n_hospitals > 1 & no_or == 0) {
  # Keep the formula as is
  model_form_step <- model_form_step
} else if (n_hospitals > 1 & no_or == 1) {
  # Remove 'or_before_enrollment'
  model_form_step <- update(model_form_step, . ~ . - or_before_enrollment)
} else if (n_hospitals == 1 & no_or == 0) {
  # Remove 'factor(hospital_id)'
  model_form_step <- update(model_form_step, . ~ . - factor(hospital_id))
} else if (n_hospitals == 1 & no_or == 1) {
  # Remove both 'factor(hospital_id)' and 'or_before_enrollment'
  model_form_step <- update(model_form_step, . ~ . - factor(hospital_id) - or_before_enrollment)
}
...
#This is the Model Form for Evaluating the Step Changes AND Time Trends
#Will Scale STudy Month So Coefficients Tell Us About Change in Log Odds Per 3 month Change
model_form_step <- prone_12hour_outcome ~ study_period + study_period:month_scaled + age_scale + female + bmi_scale + factor(nee_pressor_dose) + sofa_score_scale + min_pf_ratio_scale + or_before_enrollment + factor(hospital_id)
...
#Output a Table of Average Marginal Effects Using the AME Function Created Above
model_list <- list(multivar_mod, multivar_mod_interact)
...
#Full Model
model_form_step <- prone_12hour_outcome ~ covid_period + covid_period:month_scaled + age_scale + female +
  bmi_scale + or_before_enrollment + factor(nee_pressor_dose) + sofa_score_scale +  min_pf_ratio_scale + factor(hospital_id)
...
model_form_step <- prone_12hour_outcome ~ month_scaled + age_scale + female + bmi_scale + factor(nee_pressor_dose) + sofa_score_scale + min_pf_ratio_scale + or_before_enrollment + factor(hospital_id)
#Now Multivariable
if (n_hospitals > 1 & no_or == 0) {
  # Keep the formula as is
  model_form_step <- model_form_step
...
model_form_step <- prone_24hour_outcome ~ study_period + age_scale + female + bmi_scale + factor(nee_pressor_dose) + sofa_score_scale + min_pf_ratio_scale + or_before_enrollment + factor(hospital_id)
...
#This is the Model Form for Evaluating the Step Changes AND Time Trends
#Will Scale STudy Month So Coefficients Tell Us About Change in Log Odds Per 3 month Change
model_form_step <- prone_24hour_outcome ~ study_period + study_period:month_scaled + age_scale + female + bmi_scale + factor(nee_pressor_dose) + sofa_score_scale + or_before_enrollment + min_pf_ratio_scale + factor(hospital_id)
...
model_form_step <- prone_24hour_outcome ~ covid_period + covid_period:month_scaled + age_scale + female + bmi_scale + factor(nee_pressor_dose) + sofa_score_scale +  min_pf_ratio_scale + or_before_enrollment + factor(hospital_id)
...
model_form_step <- prone_24hour_outcome ~ month_scaled + age_scale + female + bmi_scale + factor(nee_pressor_dose) + sofa_score_scale +  min_pf_ratio_scale + or_before_enrollment + factor(hospital_id)
#Now Multivariable
if (n_hospitals > 1 & no_or == 0) {
  # Keep the formula as is
  model_form_step <- model_form_step
...
#Full Model
model_form_step <- prone_72hour_outcome ~ study_period + age_scale + female + bmi + factor(nee_pressor_dose) + sofa_score_scale + min_pf_ratio_scale + or_before_enrollment + factor(hospital_id)
...
#This is the Model Form for Evaluating the Step Changes AND Time Trends
#Will Scale STudy Month So Coefficients Tell Us About Change in Log Odds Per 3 month Change
model_form_step <- prone_72hour_outcome ~ study_period + study_period:month_scaled + age_scale + female + bmi_scale + factor(nee_pressor_dose) + sofa_score_scale +  min_pf_ratio_scale + or_before_enrollment + factor(hospital_id) 
...
model_form_step <- prone_72hour_outcome ~ covid_period + covid_period:month_scaled + age_scale + female + bmi_scale + factor(nee_pressor_dose) + sofa_score_scale + min_pf_ratio_scale + or_before_enrollment + factor(hospital_id)
...
#Now Multivariable
model_form_step <- prone_72hour_outcome ~ month_scaled + age_scale + female + bmi_scale + factor(nee_pressor_dose) + sofa_score_scale + min_pf_ratio_scale + or_before_enrollment + factor(hospital_id)
...
model_form_step <- proned ~ study_period + age_scale + female + bmi_scale + factor(nee_pressor_dose) + sofa_score_scale + min_pf_ratio_scale + or_before_enrollment + factor(hospital_id)
...
#This is the Model Form for Evaluating the Step Changes AND Time Trends
#Will Scale STudy Month So Coefficients Tell Us About Change in Log Odds Per 3 month Change
model_form_step <- proned ~ study_period + study_period:month_scaled + age_scale + female + bmi_scale + factor(nee_pressor_dose) + sofa_score_scale + min_pf_ratio_scale + or_before_enrollment + factor(hospital_id)
...
model_form_step <- proned ~ covid_period + covid_period:month_scaled + age_scale + female + bmi_scale + factor(nee_pressor_dose) + sofa_score_scale + min_pf_ratio_scale + or_before_enrollment + factor(hospital_id)
...
#Now Multivariable
model_form_step <- proned ~ month_scaled + age_scale + female + bmi_scale + factor(nee_pressor_dose) + sofa_score+ min_pf_ratio_scale + or_before_enrollment + factor(hospital_id)
if (n_hospitals > 1 & no_or == 0) {
  # Keep the formula as is
  model_form_step <- model_form_step
} else if (n_hospitals > 1 & no_or == 1) {
...
alt_form <- prone_12hour_outcome ~ age_scale  + female +
  bmi + factor(nee_pressor_dose) +sofa_score_scale+ or_before_enrollment + min_pf_ratio_scale

#Adjust if No OR Before Enrollment
if (no_or == 0) {
  # Keep the formula as is
  alt_form <- alt_form
} else if (no_or == 1) {
  # Remove 'or_before_enrollment'
  alt_form <- update(alt_form, . ~ . - or_before_enrollment)
} 
