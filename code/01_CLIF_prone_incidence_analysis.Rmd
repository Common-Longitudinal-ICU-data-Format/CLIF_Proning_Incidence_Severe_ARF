...
project_location <- 'z:/barker/clif_temp/output/proning'
site <- "U Michigan"
```
...
if (n_hospitals>1) {
model_form_step <- prone_12hour_outcome ~ study_period + age_at_admission  + bmi + factor(max_norepi_equivalent) +  min_pf_ratio + factor(hospital_id)
} else {
  model_form_step <- prone_12hour_outcome ~ study_period + age_at_admission  + bmi + factor(max_norepi_equivalent) +  min_pf_ratio
}
...
#Will Scale STudy Month So Coefficients Tell Us About Change in Log Odds Per 3 month Change
if (n_hospitals>1) {
model_form_step <- prone_12hour_outcome ~ study_period + study_period:month_scaled + age_at_admission + bmi + factor(max_norepi_equivalent) +  min_pf_ratio + factor(hospital_id) 
} else {
  model_form_step <- prone_12hour_outcome ~ study_period + study_period:month_scaled + age_at_admission + bmi + factor(max_norepi_equivalent) +  min_pf_ratio
}
...
if (n_hospitals>1) {
  model_form_step <- prone_12hour_outcome ~ covid_period + covid_period:month_scaled + age_at_admission+ bmi + factor(max_norepi_equivalent) +  min_pf_ratio + factor(hospital_id)
} else {
model_form_step <- prone_12hour_outcome ~ covid_period + covid_period:month_scaled + age_at_admission+ bmi + factor(max_norepi_equivalent) +  min_pf_ratio 
}
...
#Now Multivariable
if (n_hospitals>1) {
model_form_step <- prone_12hour_outcome ~ month_scaled + age_at_admission  + bmi + factor(max_norepi_equivalent) +  min_pf_ratio + factor(hospital_id)
} else {
  model_form_step <- prone_12hour_outcome ~ month_scaled + age_at_admission  + bmi + factor(max_norepi_equivalent) +  min_pf_ratio
}
two_period_multivar_mod <- glm(model_form_step,
          data = prone_analytic_df, 
          family=binomial)
...
```{r Segmented Regression - Univariable - 24h}
...
```{r Adjusted Segmented Regression - Proning within 24h}
if (n_hospitals>1) {
model_form_step <- prone_24hour_outcome ~ study_period + age_at_admission  + bmi + factor(max_norepi_equivalent) +  min_pf_ratio + factor(hospital_id)
} else {
  model_form_step <- prone_24hour_outcome ~ study_period + age_at_admission  + bmi + factor(max_norepi_equivalent) +  min_pf_ratio
}
...
#Will Scale STudy Month So Coefficients Tell Us About Change in Log Odds Per 3 month Change
if (n_hospitals>1) {
model_form_step <- prone_24hour_outcome ~ study_period + study_period:month_scaled + age_at_admission + bmi + factor(max_norepi_equivalent) +  min_pf_ratio + factor(hospital_id) 
} else {
  model_form_step <- prone_24hour_outcome ~ study_period + study_period:month_scaled + age_at_admission + bmi + factor(max_norepi_equivalent) +  min_pf_ratio
}
...
```{r Post HOC Analysis Analyzing Just Step Change at Transition to COVID Period With Rate of Change Prior and After - 24h proning}
#For Sites with PRE-COVID Data They Will A STep Change From Pre-COVID to COVID and then analyze effect of time (in 3 month interval before and after this step change)
#For Sites without Pre-COVID Data There is No Period Change and the Interest is in the Rate of Proning Decline Over the Study Period
#All prones 
prone_analytic_df <- prone_analytic_df |>
...
if (n_hospitals>1) {
  model_form_step <- prone_24hour_outcome ~ covid_period + covid_period:month_scaled + age_at_admission+ bmi + factor(max_norepi_equivalent) +  min_pf_ratio + factor(hospital_id)
} else {
model_form_step <- prone_24hour_outcome ~ covid_period + covid_period:month_scaled + age_at_admission+ bmi + factor(max_norepi_equivalent) +  min_pf_ratio 
}
...
#Now Multivariable
if (n_hospitals>1) {
model_form_step <- prone_24hour_outcome ~ month_scaled + age_at_admission  + bmi + factor(max_norepi_equivalent) +  min_pf_ratio + factor(hospital_id)
} else {
  model_form_step <- prone_24hour_outcome ~ month_scaled + age_at_admission  + bmi + factor(max_norepi_equivalent) +  min_pf_ratio
}
two_period_multivar_mod <- glm(model_form_step,
          data = prone_analytic_df, 
          family=binomial)
...
```{r Segmented Regression - Univariable - Proning within 72 hours}
#For Sites with > 1 Hospital, Run Mixed Effects Models Otherwise Run as Logistic Regression
#For now these are Logistic Regressions
#This is the Model Form for Evaluating the Step Changes NOT taking Into Account Time Trends
#Look at all proning outcome. 
...
```{r Adjusted Segmented Regression - Proning within 72 hours}
if (n_hospitals>1) {
model_form_step <- prone_72hour_outcome ~ study_period + age_at_admission  + bmi + factor(max_norepi_equivalent) +  min_pf_ratio + factor(hospital_id)
} else {
  model_form_step <- prone_72hour_outcome ~ study_period + age_at_admission  + bmi + factor(max_norepi_equivalent) +  min_pf_ratio
}
...
#Will Scale STudy Month So Coefficients Tell Us About Change in Log Odds Per 3 month Change
if (n_hospitals>1) {
model_form_step <- prone_72hour_outcome ~ study_period + study_period:month_scaled + age_at_admission + bmi + factor(max_norepi_equivalent) +  min_pf_ratio + factor(hospital_id) 
} else {
  model_form_step <- prone_72hour_outcome ~ study_period + study_period:month_scaled + age_at_admission + bmi + factor(max_norepi_equivalent) +  min_pf_ratio
}
...
```{r Post HOC Analysis Analyzing Just Step Change at Transition to COVID Period With Rate of Change Prior and After - 72h proning}
#For Sites with PRE-COVID Data They Will A STep Change From Pre-COVID to COVID and then analyze effect of time (in 3 month interval before and after this step change)
#For Sites without Pre-COVID Data There is No Period Change and the Interest is in the Rate of Proning Decline Over the Study Period
#All prones 
prone_analytic_df <- prone_analytic_df |>
...
if (n_hospitals>1) {
  model_form_step <- prone_72hour_outcome ~ covid_period + covid_period:month_scaled + age_at_admission+ bmi + factor(max_norepi_equivalent) +  min_pf_ratio + factor(hospital_id)
} else {
model_form_step <- prone_72hour_outcome ~ covid_period + covid_period:month_scaled + age_at_admission+ bmi + factor(max_norepi_equivalent) +  min_pf_ratio 
}
...
#Now Multivariable
if (n_hospitals>1) {
model_form_step <- prone_72hour_outcome ~ month_scaled + age_at_admission  + bmi + factor(max_norepi_equivalent) +  min_pf_ratio + factor(hospital_id)
} else {
  model_form_step <- prone_72hour_outcome ~ month_scaled + age_at_admission  + bmi + factor(max_norepi_equivalent) +  min_pf_ratio
}
two_period_multivar_mod <- glm(model_form_step,
          data = prone_analytic_df, 
          family=binomial)
...
if (n_hospitals>1) {
model_form_step <- proned ~ study_period + age_at_admission  + bmi + factor(max_norepi_equivalent) +  min_pf_ratio + factor(hospital_id)
} else {
  model_form_step <- proned ~ study_period + age_at_admission  + bmi + factor(max_norepi_equivalent) +  min_pf_ratio
}
...
#Will Scale STudy Month So Coefficients Tell Us About Change in Log Odds Per 3 month Change
if (n_hospitals>1) {
model_form_step <- proned ~ study_period + study_period:month_scaled + age_at_admission + bmi + factor(max_norepi_equivalent) +  min_pf_ratio + factor(hospital_id) 
} else {
  model_form_step <- proned ~ study_period + study_period:month_scaled + age_at_admission + bmi + factor(max_norepi_equivalent) +  min_pf_ratio
}
...
if (n_hospitals>1) {
  model_form_step <- proned ~ covid_period + covid_period:month_scaled + age_at_admission+ bmi + factor(max_norepi_equivalent) +  min_pf_ratio + factor(hospital_id)
} else {
model_form_step <- proned ~ covid_period + covid_period:month_scaled + age_at_admission+ bmi + factor(max_norepi_equivalent) +  min_pf_ratio 
}
...
#Now Multivariable
if (n_hospitals>1) {
model_form_step <- proned ~ month_scaled + age_at_admission  + bmi + factor(max_norepi_equivalent) +  min_pf_ratio + factor(hospital_id)
} else {
  model_form_step <- proned ~ month_scaled + age_at_admission  + bmi + factor(max_norepi_equivalent) +  min_pf_ratio
}
two_period_multivar_mod <- glm(model_form_step,
          data = prone_analytic_df, 
          family=binomial)
