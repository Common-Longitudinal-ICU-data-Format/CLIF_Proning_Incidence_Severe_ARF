...
project_location <- 'Z:/barker/clif_temp/Proning_Project'
site <- "Michigan"
file_type <- "parquet"
```
...
#Many Sites Have Zero Patients Who were Proned and Where In OR Prior to Enrollment; For Those Sites that Variable is Not Included in the Model
...
no_or <- if (no_or$or_num<10 | no_or$prone_after_or<10) {
  1
} else {
  0
}
...
#Full Model
model_form_step <- prone_12hour_outcome ~ study_period + age_at_admission + female + bmi + factor(nee_pressor_dose) + sofa_score +  min_pf_ratio + or_before_enrollment + factor(hospital_id)
...
#This is the Model Form for Evaluating the Step Changes AND Time Trends
#Will Scale STudy Month So Coefficients Tell Us About Change in Log Odds Per 3 month Change
model_form_step <- prone_12hour_outcome ~ study_period + study_period:month_scaled + age_at_admission + female + bmi + factor(nee_pressor_dose) +sofa_score+   min_pf_ratio + or_before_enrollment + factor(hospital_id) 
if (n_hospitals > 1 & no_or == 0) {
  # Keep the formula as is
  model_form_step <- model_form_step
} else if (n_hospitals > 1 & no_or == 1) {
...
#Full Model
model_form_step <- prone_12hour_outcome ~ covid_period + covid_period:month_scaled + age_at_admission + female +
  bmi + or_before_enrollment + factor(nee_pressor_dose) + sofa_score+  min_pf_ratio + factor(hospital_id)
...
model_form_step <- prone_12hour_outcome ~ month_scaled + age_at_admission + female + bmi + factor(nee_pressor_dose) + sofa_score+   min_pf_ratio + or_before_enrollment + factor(hospital_id)
#Now Multivariable
if (n_hospitals > 1 & no_or == 0) {
  # Keep the formula as is
  model_form_step <- model_form_step
...
model_form_step <- prone_24hour_outcome ~ study_period + age_at_admission + female + bmi + factor(nee_pressor_dose) + sofa_score+ min_pf_ratio + or_before_enrollment + factor(hospital_id)
...
#This is the Model Form for Evaluating the Step Changes AND Time Trends
#Will Scale STudy Month So Coefficients Tell Us About Change in Log Odds Per 3 month Change
model_form_step <- prone_24hour_outcome ~ study_period + study_period:month_scaled + age_at_admission + female + bmi + factor(nee_pressor_dose) +sofa_score+  or_before_enrollment + min_pf_ratio + factor(hospital_id)
...
model_form_step <- prone_24hour_outcome ~ covid_period + covid_period:month_scaled + age_at_admission + female + bmi + factor(nee_pressor_dose) + sofa_score+  min_pf_ratio + or_before_enrollment + factor(hospital_id)
...
model_form_step <- prone_24hour_outcome ~ month_scaled + age_at_admission + female + bmi + factor(nee_pressor_dose) + sofa_score+  min_pf_ratio + or_before_enrollment + factor(hospital_id)
#Now Multivariable
if (n_hospitals > 1 & no_or == 0) {
  # Keep the formula as is
  model_form_step <- model_form_step
...
#Full Model
model_form_step <- prone_72hour_outcome ~ study_period + age_at_admission + female + bmi + factor(nee_pressor_dose) + sofa_score+ min_pf_ratio + or_before_enrollment + factor(hospital_id)
...
#This is the Model Form for Evaluating the Step Changes AND Time Trends
#Will Scale STudy Month So Coefficients Tell Us About Change in Log Odds Per 3 month Change
model_form_step <- prone_72hour_outcome ~ study_period + study_period:month_scaled + age_at_admission + female + bmi + factor(nee_pressor_dose) + sofa_score+  min_pf_ratio + or_before_enrollment + factor(hospital_id) 
...
model_form_step <- prone_72hour_outcome ~ covid_period + covid_period:month_scaled + age_at_admission + female + bmi + factor(nee_pressor_dose) + sofa_score+ min_pf_ratio + or_before_enrollment + factor(hospital_id)
...
#Now Multivariable
model_form_step <- prone_72hour_outcome ~ month_scaled + age_at_admission + female + bmi + factor(nee_pressor_dose) + sofa_score+ min_pf_ratio + or_before_enrollment + factor(hospital_id)
...
model_form_step <- proned ~ study_period + age_at_admission + female + bmi + factor(nee_pressor_dose) + sofa_score+ min_pf_ratio + or_before_enrollment + factor(hospital_id)
...
#This is the Model Form for Evaluating the Step Changes AND Time Trends
#Will Scale STudy Month So Coefficients Tell Us About Change in Log Odds Per 3 month Change
model_form_step <- proned ~ study_period + study_period:month_scaled + age_at_admission + female + bmi + factor(nee_pressor_dose) + sofa_score+ min_pf_ratio + or_before_enrollment + factor(hospital_id)
...
model_form_step <- proned ~ covid_period + covid_period:month_scaled + age_at_admission + female + bmi + factor(nee_pressor_dose) + sofa_score+ min_pf_ratio + or_before_enrollment + factor(hospital_id)
...
#Now Multivariable
model_form_step <- proned ~ month_scaled + age_at_admission + female + bmi + factor(nee_pressor_dose) + sofa_score+ min_pf_ratio + or_before_enrollment + factor(hospital_id)
if (n_hospitals > 1 & no_or == 0) {
  # Keep the formula as is
  model_form_step <- model_form_step
} else if (n_hospitals > 1 & no_or == 1) {
...
alt_form <- prone_12hour_outcome ~ age_at_admission  + female +
  bmi + factor(nee_pressor_dose) +sofa_score+ or_before_enrollment + min_pf_ratio
