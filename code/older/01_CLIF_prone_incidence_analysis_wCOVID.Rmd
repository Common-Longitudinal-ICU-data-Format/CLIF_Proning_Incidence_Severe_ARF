...
#First look at primary outcome proning within 12 hours by Study Period. Then look at all proning. 
if (n_hospitals>1) {
...
univar_table <- bind_rows(
  ...
  data.frame(site = c("", "Model With Interaction Terms")),
...
model_list <- list(unvar_mod, unvar_mod_sarscov2, unvar_mod_interact)
...
#Full Model - with sars_cov2 stratification
...
#Output a Table of Average Marginal Effects Using the AME Function Created Above
model_list <- list(multivar_mod, multivar_mod_sarscov2, multivar_mod_interact)
...
#For Sites with PRE-COVID Data They Will A STep Change From Pre-COVID to COVID and then analyze effect of time (in 3 month intervals before and after this step change)
...
# Write the table to a CSV file
write.csv(two_period_table, paste0(project_location, '/project_output/', site, '_two_period_models.csv'))
...
two_period_table <- rowbind(two_period_unvar_sum, data.frame('site'=c('','Adjusted Model')), fill = TRUE) |>
               rbind(two_period_multivar_sum)
# Write the table to a CSV file
write.csv(two_period_table, paste0(project_location, '/project_output/', site, '_two_period_models_24.csv'))
...
two_period_table <- rowbind(two_period_unvar_sum, data.frame('site'=c('','Adjusted Model')), fill = TRUE) |>
               rbind(two_period_multivar_sum)
# Write the table to a CSV file
write.csv(two_period_table, paste0(project_location, '/project_output/', site, '_two_period_models_72.csv'))
...
two_period_table <- rowbind(two_period_unvar_sum, data.frame('site'=c('','Adjusted Model')), fill = TRUE) |>
               rbind(two_period_multivar_sum)
# Write the table to a CSV file
write.csv(two_period_table, paste0(project_location, '/project_output/', site, '_two_period_models_alltime.csv'))
