...
#Proning By Hospital Size (BEFORE FILTERING SMALL HOSPITALS OUT)
df_hospsize <- df_agg |>
...
cat('\nProning Counts by Hospital Size and Period [Before Filtering Small Hospitals')
df_hospsize
...
stacked2 <- ggplot(stacked_data, 
                              aes(x = hospital_rank,
                                  y = adjust_rate)) +
  geom_line(aes(x = hospital_rank,
                y = adjust_rate, group=study_period, linetype=study_period),  color = 'lightgrey') +
  ...
  scale_shape_discrete(
    name ='Study Period',
    labels = c("Pandemic", "Post-pandemic", "Pre-pandemic"),
    guide = guide_legend(order = 1)
  ) +
  scale_linetype_manual(
    name = "Study Period",
    values = c("COVID" = "solid",           # Solid line
               "Post-COVID" = "dashed",     # Dashed line
               "Pre-COVID" = "twodash"),     # Dotted line
    labels = c("Pandemic", "Post-pandemic", "Pre-pandemic"),
    guide = 'none'
  ) +
  scale_color_manual(
    name = NULL,  # Empty string removes the title
    values = met.brewer("Demuth", n = 2),
    labels = c("Academic Hospital", "Community Hospital"),
...
ggsave(
  filename = 'stacked_caterpillar2.tiff',
  path = paste0(project_location, '/summary_output/graphs/'),
  width = 12,
  height = 6,
  units = "in",
...
ggsave(
  filename = 'stacked_caterpillar2.eps',
  path = paste0(project_location, '/summary_output/graphs/'),
  width = 12,       # adjust width to make it wide
  height = 6,       # adjust height for PowerPoint-friendly proportions
  units = "in",      # units set to inches for precise control
...
agg_brm_period_size <- brm(
  bf(observed_prone | trials(n_patients) ~ 
       0 + offset(prone_log_odds) + study_period + study_period * icu_bed_cat +
     (0 + study_period | hospital_id)), 
  data = df_agg,
  family = binomial,
  prior = priors, 
...
full_posterior <- as_draws_df(agg_brm_period_size)  |>
  mutate(pre_covid_med = exp(b_icu_bed_catmedium + `b_study_periodPreMCOVID:icu_bed_catmedium`), 
           pre_covid_large = exp(b_icu_bed_catlarge + `b_study_periodPreMCOVID:icu_bed_catlarge`),
         covid_med = exp(b_icu_bed_catmedium),
         covid_large = exp(b_icu_bed_catlarge),
      post_covid_med = exp(b_icu_bed_catmedium + `b_study_periodPostMCOVID:icu_bed_catmedium`),
      post_covid_large = exp(b_icu_bed_catlarge + `b_study_periodPostMCOVID:icu_bed_catlarge`))

summary_posterior_size <- data.frame(
  variable = c(
    'Pre_Covid Medium vs Small ICU Capacity',
    'Pre_Covid Large vs Small ICU Capacity',
    'Covid Medium vs Small ICU Capacity',
    'Covid Large vs Small ICU Capacity',
    'Post_Covid Medium vs Small ICU Capacity',
    'Post_Covid Large vs Small ICU Capacity'
  ),
  median_or = c(median(full_posterior$pre_covid_med),
              median(full_posterior$pre_covid_large),
              median(full_posterior$covid_med),
              median(full_posterior$covid_large),
              median(full_posterior$post_covid_med),
              median(full_posterior$post_covid_large)),
  conf.low = c(quantile(full_posterior$pre_covid_med, p=0.025),
               quantile(full_posterior$pre_covid_large, p=0.025),
               quantile(full_posterior$covid_med, p=0.025),
               quantile(full_posterior$covid_large, p=0.025),
               quantile(full_posterior$post_covid_med, p=0.025),
               quantile(full_posterior$post_covid_large, p=0.025)),
  conf.high = c(quantile(full_posterior$pre_covid_med, p=0.975),
               quantile(full_posterior$pre_covid_large, p=0.975),
               quantile(full_posterior$covid_med, p=0.975),
               quantile(full_posterior$covid_large, p=0.975),
               quantile(full_posterior$post_covid_med, p=0.975),
               quantile(full_posterior$post_covid_large, p=0.975)))
