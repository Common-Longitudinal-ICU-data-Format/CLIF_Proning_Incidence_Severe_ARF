...
tables_location <-"/projects/CLIF-analysis/critcare_rclif_2-1/critcare_rclif_2_1_tables" #Add local path to CLIF tables
project_location <-"/projects/CLIF-analysis/clif_projects/CLIF_Prone_Parent/CLIF_Proning_Incidence_Severe_ARF" #Add local path to project location, where results will output
site <-"Hopkins" #Add local site name
file_type <-"parquet" #Options are "parquet" or "csv"
#Path to Datasets from Initial Prone Project Run (last run in June 2025)
original_analytic_data <- '/projects/CLIF-analysis/clif_projects/CLIF_Prone_Parent/CCM_R0'
...
prone_analytic_df <- read_parquet(paste0(original_analytic_data, '/prone_analytic_data.parquet')) 

#Apply Function to Convert to Local Timezone

#Load Cohort_ids
load(paste0(original_analytic_data, '/cohort_hospitalization_ids.RData')) 
...
vent_practices <- clif_resp |>
  ...
  mutate(
   ...
   study_height_cm=fcase(
     study_height_cm>243 | study_height_cm<76, NaN,
     default=study_height_cm),
...
vent_practices <- vent_practices |>
  ...
  mutate(
    tidal_volume=fcase(
      tidal_volume>1500, NaN, 
      tidal_volume<150, NaN,
      default =tidal_volume
    ),
...
med_continuous <- open_dataset(paste0(tables_location, '/clif_medication_admin_continuous.parquet')) |>
  ...
  filter(tolower(med_group) %in% c(
    ...
    tolower(med_route_category) %in% c('intravenous', 'inhaled'),
    !is.na(med_dose),
