...
# Detach tidytable if it is loaded
if ("package:tidytable" %in% search()) {
  detach("package:tidytable", unload = TRUE)
  cat("tidytable has been detached\n")
} else {
  cat("tidytable was not loaded\n")
}
...
fn.timezone_tolocal <- function(df) {
  ...
  df <- df %>%
    mutate(dplyr::across(dplyr::all_of(datetime_cols), ~ cast(.x, arrow::timestamp("us", site_time_zone)))) |>
    compute()
}
```
...
```{r}
ventilation_icd <- open_dataset(paste0(tables_location, "/", larger_proning_table, ".parquet"))|>
  mutate(hospitalization_id = as.character(hospitalization_id)) |>
  dplyr::semi_join(cohort_hospitalization_ids, by = "hospitalization_id") |>
  dplyr::left_join(
    cohort_hospitalization_ids |>
...
joined_df$any_ventilation_flowsheet = 1
...
icd_only_summary
write.csv(icd_only_summary,
         file = paste0(project_location, '/project_output/revision/icd/', site, '_icd_only_summary.csv'),row.names = FALSE)

cat("\nICD-only summary data saved as '",
    paste0(project_location, '/project_output/revision/icd/', site, '_icd_only_summary.csv'),"'\n", sep = "")
```
