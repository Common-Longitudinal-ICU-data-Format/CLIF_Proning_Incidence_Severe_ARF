...
pao2 <- clif_labs |>
  ...
  collect() |>
  mutate(recorded_dttm=as.POSIXct(lab_result_dttm, tz="UTC")) |> #For Merging with Vent Data
...
hospital_location <- clif_adt |>
  ...
  mutate(or_before_enrollment=fifelse(
    tolower(location_category) =='procedural' & 
      as.POSIXct(in_dttm, tz="UTC")>=as.POSIXct(t_enrollment, tz="UTC")-ddays(7) &
...
prone_analytic_data <- hospitalization_vars |>
  ...
  mutate(race_ethnicity=fcase(
    ...
    (tolower(race_category)=='unknown'| is.na(race_category)), 'unknown')) |>
