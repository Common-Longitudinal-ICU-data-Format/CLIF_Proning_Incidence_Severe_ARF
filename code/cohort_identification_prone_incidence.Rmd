...
proseva_criteria <- vent_data |>
  ...
  mutate(eligible_proseva_t2=fifelse(
    ...
    recorded_dttm<=as.POSIXct(t_proseva_first, tz="UTC")+dhours(24), 1, 0
  )) |>
...
norepi_equivalent <- clif_medication_admin_continuous |>
  ...
  #Norepi Equivalents: 1=1 low dose pressor, 2=1 moderate or high dose pressor, 3=2 or more pressors, 0 is 0 pressors
   mutate(norepi_equivalent=case_when( #norepi_dose + epi_dose+ vaso_dose*2.5/60+ angiotensin_dose/100+ dopa_dose/100+ phenyl_dose/10)|>
       (norepi_dose<=0.15 | epi_dose <= 0.15 | 
          phenyl_dose <= 1.5 | angiotensin_dose/100<=0.15) & pressor_on==1 ~ 1,
       (norepi_dose> 0.15 | epi_dose > 0.15 | 
          phenyl_dose > 1.5 | dopa_dose > 22 | angiotensin_dose/100>0.15) & pressor_on==1 ~ 2,
       (vaso_dose<=0.04 | vaso_dose/60 <=0.04) & pressor_on==1 ~ 1,
       pressor_on > 1 ~ 2,
       dopa_dose <= 22 & pressor_on==1 ~ 1,
       pressor_on==0 ~ 0)) |>
   #Now Select Max In this 12 Hour Period
   group_by(patient_id, encounter_block) |>
   mutate(max_norepi_equivalent=max(norepi_equivalent, na.rm = TRUE)) |>
   filter(row_number()==1) |>
   select(patient_id, encounter_block, max_norepi_equivalent, norepi_dose, epi_dose, vaso_dose, angiotensin_dose, dopa_dose, phenyl_dose) |>
   ungroup()
