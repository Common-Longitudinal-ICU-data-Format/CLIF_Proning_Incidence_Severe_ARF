...
hospitalization_vars <- clif_hospitalization |>
  ...
  mutate(in_hosp_death=fifelse(
    final_discharge_category=='Expired', 1, 0
  )) |>
  mutate(death_or_hospice=fifelse(
    final_discharge_category %in% c('Expired', 'Hospice'), 1, 0
  )) 
...
hospital_location <- clif_adt |>
  ...
  mutate(or_before_enrollment=fifelse(
    tolower(location_category) =='procedural' & 
      as.POSIXct(in_dttm)>=as.POSIXct(t_enrollment)-ddays(7) &
      as.POSIXct(in_dttm)<=as.POSIXct(t_enrollment), 1, 0
  )) |>
  group_by(patient_id, encounter_block) |>
  mutate(or_before_enrollment=max(or_before_enrollment, na.rm = TRUE)) |>
  ungroup() |>
  filter(enrollment_icu==1) |>
  select(patient_id, encounter_block, location_category, location_name, or_before_enrollment) |>
...
#Calculate BMI
bmi <- clif_vitals |>
  filter(vital_category %in% c('weight_kg', 'height_inches')) |>
  filter(hospitalization_id %in% cohort_hospitalization_ids$hospitalization_id) |>
  #Filter Outliers
  filter((vital_category=='height_inches' & vital_value>=30 & vital_value<=96) |
  ...
  group_by(patient_id, encounter_block) |>
  mutate(study_height_inches=fcase(
    keep==1 & before_enrollment==1 & vital_category=='height_inches', vital_value
  )) |>
  mutate(study_weight_kg=fcase(
      keep==1 & before_enrollment==1 & vital_category=='weight_kg', vital_value
  )) |>
  fill(study_height_inches, study_weight_kg, .direction = 'downup') |>
  #IF missing Can Use First Value After Enrollment
  mutate(study_height_inches=fifelse(
    is.na(study_height_inches) & keep==1 & 
      before_enrollment==0 & vital_category=='height_inches', 
    vital_value, study_height_inches
  )) |>
  ...
  ungroup() |>
  #Calculate BMI
  mutate(bmi=study_weight_kg/((study_height_inches/39.37)^2)) |>
  select(patient_id, encounter_block, study_height_inches, study_weight_kg, bmi)
...
min_pf <- vent_data |>
  ...
  filter(as.POSIXct(recorded_dttm)>=as.POSIXct(vent_episode_start) &
...
norepi_equivalent <- clif_medication_admin_continuous |>
  filter(hospitalization_id %in% cohort_hospitalization_ids$hospitalization_id) |>
  left_join(cohort_hospitalization_ids) |>
  filter(in_cohort==1) |>
  ...
  collect() |>
  filter(as.POSIXct(admin_dttm)>=as.POSIXct(t_enrollment)-dhours(12) & 
  ...
  mutate(epi_dose=fifelse(is.na(epi_dose), 0, norepi_dose)) |>
  ...
  ungroup() |>
  #See Lambden et al for Norepi dose equivalents for Epi, Phenyl, Vaso, Dopa, Angiotensin)
  mutate(pressor_no=(norepi_on + epi_on + phenyl_on + vaso_on + dopa_on + angiotensin_on)) |>
  #Norepi Equivalents: 1=1 low dose pressor, 2=1 moderate or high dose pressor, 3=2 or more pressors, 0 is 0 pressors
   mutate(norepi_equivalent=case_when(
      (norepi_dose<=0.15 | epi_dose <= 0.15 | 
         phenyl_dose <= 1.5 | angiotensin_dose/100<=0.15) & pressor_no==1 ~ 1,
      (norepi_dose> 0.15 | epi_dose > 0.15 | 
         phenyl_dose > 1.5 | dopa_dose > 22 | angiotensin_dose/100>0.15) & pressor_no==1 ~ 2,
      vaso_dose<=0.04 & pressor_no==1 ~ 1,
      pressor_no > 1 ~ 2,
      dopa_dose <= 22 & pressor_no==1 ~ 1,
      pressor_no==0 ~ 0)) |>
   #Now Select Max In this 12 Hour Period
   group_by(patient_id, encounter_block) |>
   mutate(max_norepi_equivalent=max(norepi_equivalent, na.rm = TRUE)) |>
   filter(row_number()==1) |>
   select(patient_id, encounter_block, max_norepi_equivalent) |>
   ungroup()
...
prone_analytic_data <- hospitalization_vars |>
  ...
  mutate(max_norepi_equivalent=fifelse(is.na(max_norepi_equivalent), 0, max_norepi_equivalent)) |>
  select(-hospitalization_id) |>
  ...
  mutate(race_ethnicity=fcase(
    ...
    tolower(race_category)=='unknown', 'unknown')) |>
...
clif_patient_cohort <- clif_patient |>
  filter(patient_id %in% cohort_patient_ids$patient_id) |>
  collect()
```
