...
print(tab1)
tab1_excel <- print(tab1, printToggle = FALSE)
...
summary(unvar_mod)
exp(confint(unvar_mod))

#This is the Model Form for Evaluating the Step Chanegs AND Time Trends
if (n_hospitals>1) {
  model_form_step <- prone_12hour_outcome ~ study_period + study_period:study_month + (1 | hospital_id)
  unvar_mod_interact <- glmmTMB(model_form_step,
          data = prone_analytic_df, 
          family=binomial)
} else {
  model_form_step <- prone_12hour_outcome ~ study_period + study_period:study_month
...
exp(confint(unvar_mod_interact))
```
