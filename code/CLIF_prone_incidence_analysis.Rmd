...
if (model_summary$ngrps$cond[1]> 1) {
...
model_form_step <- prone_12hour_outcome ~ study_period + age_at_admission + female + 
  race_ethnicity + bmi + factor(max_norepi_equivalent) + or_before_enrollment + min_pf_ratio + (1 | hospital_id)
...
model_form_step <- prone_12hour_outcome ~ month_scaled + age_at_admission + female + 
  race_ethnicity + bmi + factor(max_norepi_equivalent) + or_before_enrollment + min_pf_ratio + (1 | hospital_id)
two_period_multivar_mod <- glmmTMB(model_form_step,
          data = prone_analytic_df, 
          family=binomial)
...
#For now these are Logistic Regressions
#This is the Model Form for Evaluating the Step Changes NOT taking Into Account Time Trends
#Now look at model for proning during admission "all time" regardless of time from t_enrollment. 
...
```{r Adjusted Segmented Regression}
model_form_step <- proned ~ study_period + age_at_admission + female + 
  race_ethnicity + bmi + factor(max_norepi_equivalent) + or_before_enrollment + min_pf_ratio + (1 | hospital_id)
multivar_mod <- glmmTMB(model_form_step,
          data = prone_analytic_df, 
          family=binomial)
...
#This is the Model Form for Evaluating the Step Changes AND Time Trends
#Will Scale STudy Month So Coefficients Tell Us About Change in Log Odds Per 3 month Change
model_form_step <- proned ~ study_period + study_period:month_scaled + age_at_admission + female + 
  race_ethnicity + bmi + factor(max_norepi_equivalent) + or_before_enrollment + min_pf_ratio + (1 | hospital_id)
multivar_mod_interact <- glmmTMB(model_form_step,
          data = prone_analytic_df, 
          family=binomial)
...
#For Sites with PRE-COVID Data They Will A STep Change From Pre-COVID to COVID and then analyze effect of time (in 3 month interval before and after this step change)
#For Sites without Pre-COVID Data There is No Period Change and the Interest is in the Rate of Proning Decline Over the Study Period
#Running for all proning episodes "all time" not just within 12h
...
model_form_step <- proned ~ covid_period + covid_period:month_scaled + age_at_admission + female + 
  race_ethnicity + bmi + factor(max_norepi_equivalent) + or_before_enrollment + min_pf_ratio + (1 | hospital_id)
two_period_multivar_mod <- glmmTMB(model_form_step,
          data = prone_analytic_df, 
          family=binomial)
...
model_form_step <- proned ~ month_scaled + age_at_admission + female + 
  race_ethnicity + bmi + factor(max_norepi_equivalent) + or_before_enrollment + min_pf_ratio + (1 | hospital_id)
two_period_multivar_mod <- glmmTMB(model_form_step,
          data = prone_analytic_df, 
          family=binomial)
